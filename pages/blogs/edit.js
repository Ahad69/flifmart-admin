...
const EditBlogs = ({ blogs }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios
        .patch(
          `https://flifmart-backend-v2.vercel.app/api/blog/${router?.query?.id}`,
          data,
          {
            headers: {
        ...
        <div className="bg-white m-5 p-5">
          ...
          <div className="flex">
            ...
            {fileList?.[0]?.originFileObj ? (
              ...
              <img className="w-[300px]" src={blogs?.featuredImage} />
            )}
          </div>
          <Modal
            open={previewOpen}
...
export async function getServerSideProps(context) {
  ...
  const singleBlogs = await fetch(
    `https://flifmart-backend-v2.vercel.app/api/blog/single/${query.id}`
  );

  const singleBlog = await singleBlogs.json();
  return {
    props: {
      blogs: singleBlog.blog,
    },
  };
}
